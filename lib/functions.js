...
exports.catch = function (opts) {
  return function (err, code) {
    ...
    if (!codes.isServerError(code)){
      throw new Error('Error code must be an HTTP server error code');
    }
    var body = {};
    if (opts.includeCode){
      body[opts.includeCode] = code;
...
exports.sign = function (opts) {
  return function (body, code) {
    code = parseInt(code) || 200;
    if (!codes.isOk(code)){
      throw new Error('Error code must be a successful HTTP code');
    }
    var result = {};
    if (opts.includeSuccess){
      result[opts.includeSuccess] = true;
    }
    if (opts.includeCode){
      result[opts.includeCode] = code;
    }
    result[opts.signing.tokenKey] = jwt.sign(body, opts.signing.jwtSecret);
    result[opts.signing.bodyKey] = body;
